...
**Date**: 2026-05-31  
**Status**: implemented (reconciled to the shipped code)
...
- `require_mtls_domain()` — request must arrive on the mTLS attestation domain; when
  `MTLS_PROXY_SECRET` is configured, `X-Mtls-Proxy-Auth` must match (so `X-Client-Cert` is trustworthy).
...
- `X-Client-Cert`/`X-Real-IP` are trusted only from the registry proxy. When `REGISTRY_PROXY_SECRET` is set, `require_registry_proxy_secret` rejects requests lacking the matching `X-Registry-Proxy-Auth`; the mTLS attestation endpoints are likewise gated by `require_mtls_domain`. The backend must not trust these headers on a connection that bypassed the proxy.
...
2. **`POST /servers/{vm_name}/provision` + `/provision/confirm` routes** — in the server router:
   ...
   - `provision` deps: `require_mtls_domain()`, `extract_client_cert()`, `require_luks_quote_nonce`; calls `process_provision_request` → `verify_quote(quote, nonce, SHA256(client_cert pubkey))` → `record_vm_ca_identity` (upsert `vm_root_ca_cert` = client cert PEM) → `_issue_storage_secrets`.
   - Response model: `ProvisionResponse {volumes, k3s_encryption_key, confirm_nonce}`.
   - `provision_confirm` deps: `require_mtls_domain()`, `require_confirm_nonce`; delegates to the shared `process_luks_confirm`.
...
7. **Tests** (`tests/unit/test_registry_mtls_auth.py`, `test_mtls_enforcement.py`):
   ...
   - `require_mtls_domain` / `require_registry_proxy_secret` — proxy-guard unit tests in `test_mtls_enforcement.py`
