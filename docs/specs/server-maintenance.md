...
- **Sole-survivor rule (product choice — document in PR):**
  - **Option A (recommended default):** If **any** active instance on that server is the **only** `active` instance globally for its `chute_id`, **confirm fails** with **409** and a JSON body listing blocking `{ chute_id, instance_id? }`. **No auto-terminate** in that case (network keeps last copy until another miner scales up). **Auto-bounty on deny** remains **deferred**.
  - **Option B:** Allow **confirm** even with sole survivors; **auto-terminate** must run with **`valid_termination = true`** and **explicit skip** of the last-instance `compute_multiplier` / bounty penalty for **each** purge in that batch (same semantics as a protected manual delete). This risks **capacity gaps** unless combined with strong autoscaler / bounty policy—use only if intentional.
...
- **Successful boot = end of cycle (not “wipe and allow re-confirm”):** When **boot attestation** succeeds **for the server row being upgraded** and the attested `measurement_version` is **>=** the **active** **`tee_upgrade_windows`** row’s **`target_measurement_version`** read **at boot time** (semver / `semcomp` as elsewhere—**not** the pending row’s target if the operator has published a **stricter** new floor): set **`last_maintenance_completed_at = now()`**, **`last_maintenance_completed_window_id = maintenance_pending_window_id`**, and **`last_maintenance_completed_target_measurement_version`** from the **pending window row’s** `target_measurement_version` (denormalised copy for quick checks / logs). Then **clear active-slot fields** (`maintenance_declared_at`, `maintenance_deadline_at`, **`maintenance_pending_window_id`**, confirm metadata)—**do not** clear the **completion** fields. **After that:** new preflight/confirm requires a **new** active window row, **not** already at target, and **`last_maintenance_completed_window_id ≠ active.id`** (equivalently: new row with new **`id`** after the next cutover).
...
- **Operational:** Miners **`GET …/preflight` → `POST …/confirm`**; **`GET …/policy`** reflects the **active DB row** (cached). Validators manage **rows**, not env window clocks.
