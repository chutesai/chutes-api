...
CREATE OR REPLACE FUNCTION refresh_invocation_quota(p_user_id VARCHAR)
RETURNS VOID AS $$
DECLARE
    ...
    v_lifetime_payments DOUBLE PRECISION;
BEGIN
    SELECT COALESCE(SUM(usd_amount), 0) INTO v_lifetime_payments
    FROM payments
    WHERE user_id = p_user_id;
    
    SELECT v_lifetime_payments + COALESCE(SUM(amount), 0) INTO v_lifetime_payments
    FROM admin_balance_changes
    WHERE user_id = p_user_id AND reason LIKE 'web-top%';
    
    IF v_lifetime_payments < 5.0 THEN
        RETURN;
    END IF;
...
RETURNS TRIGGER AS $$
BEGIN
    PERFORM refresh_invocation_quota(NEW.user_id);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
