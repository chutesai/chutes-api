...
# ):
#     """DEPRECATED: Use POST /launch_config/tee/{config_id} instead"""
...
async def claim_tee_launch_config(
    ...
    # Everything checks out - return symmetric key for TEE instances
    return {
...
async def claim_launch_config(
    ...
    """DEPRECATED: Use POST /launch_config/graval/{config_id} instead"""
    # TODO: Delete once legacy launch configs are no longer in ciruclation
...
async def verify_launch_config_instance(
    ...
    """DEPRECATED: Use PUT /launch_config/graval/{config_id} instead"""
    # TODO: Delete once legacy launch configs are no longer in ciruclation
    token = authorization.strip().split(" ")[-1]
    launch_config = await load_launch_config_from_jwt(db, config_id, token, allow_retrieved=True)
