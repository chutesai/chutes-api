...
from api.server.service import verify_quote
...
def test_tdx_verification_result_rtmrs_property():
    ...
    result = TdxVerificationResult(
        mrtd="a" * 96, rtmr0="0" * 96, rtmr1="1" * 96, 
        rtmr2="2" * 96, rtmr3="3" * 96,
        user_data=None, parsed_at=datetime.now(timezone.utc), is_valid=True
    )
...
def test_tdx_verification_result_to_dict():
    ...
    result = TdxVerificationResult(
        ...
        user_data="test", parsed_at=now, is_valid=True
    )
...
@pytest.mark.parametrize("minutes,expected_seconds", [
    (1, 60), (5, 300), (10, 600), (30, 1800), (60, 3600),
])
...
async def test_verify_quote_signature_success(sample_boot_quote):
    ...
    with patch('api.server.util.get_collateral_and_verify', return_value=mock_verified_report) as mock_verify:
...
def test_boot_vs_runtime_quote_differences():
    ...
    boot_quote = BootTdxQuote(
        version=4, att_key_type=2, tee_type=0x81,
        mrtd="a" * 96, rtmr0="boot_rtmr0", rtmr1="boot_rtmr1", 
        rtmr2="d" * 96, rtmr3="e" * 96,
        user_data=None, raw_quote_size=4096,
        parsed_at=datetime.now(timezone.utc).isoformat(),
        raw_bytes=b"boot"
    ...
    runtime_quote = RuntimeTdxQuote(
        version=4, att_key_type=2, tee_type=0x81,
        mrtd="a" * 96, rtmr0="runtime_rtmr0", rtmr1="runtime_rtmr1", 
        rtmr2="d" * 96, rtmr3="e" * 96,
        user_data=None, raw_quote_size=4096,
        parsed_at=datetime.now(timezone.utc).isoformat(),
        raw_bytes=b"runtime"
...
def test_quote_with_mixed_case_hex():
    ...
    quote = BootTdxQuote(
        version=4, att_key_type=2, tee_type=0x81,
        mrtd="AbCdEf" * 16, rtmr0="123456" * 16, rtmr1="fedcba" * 16, 
        rtmr2="d" * 96, rtmr3="e" * 96,
        user_data=None, raw_quote_size=4096,
        parsed_at=datetime.now(timezone.utc).isoformat(),
        raw_bytes=b"test"
...
def test_extract_nonce_with_binary_data():
    ...
    quote = BootTdxQuote(
        version=4, att_key_type=2, tee_type=0x81,
        mrtd="a" * 96, rtmr0="b" * 96, rtmr1="c" * 96, rtmr2="d" * 96, rtmr3="e" * 96,
        user_data=user_data_hex, raw_quote_size=4096,
        parsed_at=datetime.now(timezone.utc).isoformat(),
        raw_bytes=b"test"
    )
...
def test_measurement_verification_with_actual_config(mock_settings):
    ...
    from tests.fixtures.tdx import (
        EXPECTED_MRTD, EXPECTED_RMTR0, EXPECTED_RMTR1, 
        EXPECTED_RMTR2, EXPECTED_RMTR3
    )
    ...
    boot_quote = BootTdxQuote(
        version=4, att_key_type=2, tee_type=0x81,
        mrtd=EXPECTED_MRTD, rtmr0=EXPECTED_RMTR0, rtmr1=EXPECTED_RMTR1,
        rtmr2=EXPECTED_RMTR2, rtmr3=EXPECTED_RMTR3,
        user_data=None, raw_quote_size=4096,
        parsed_at=datetime.now(timezone.utc).isoformat(),
        raw_bytes=b"boot"
    )
    ...
    runtime_quote = RuntimeTdxQuote(
        version=4, att_key_type=2, tee_type=0x81,
        mrtd=EXPECTED_MRTD, rtmr0=EXPECTED_RMTR0, rtmr1=EXPECTED_RMTR1,
        rtmr2="0" * 96, rtmr3="0" * 96,  # Different runtime values
        user_data=None, raw_quote_size=4096,
        parsed_at=datetime.now(timezone.utc).isoformat(),
        raw_bytes=b"runtime"
    )
