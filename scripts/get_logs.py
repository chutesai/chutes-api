...
async def get_logs(instance_id: str = None, host: str = None, port: int = None, miner_hotkey: str = None):
    if host and port:
        if not miner_hotkey:
            if not instance_id:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--miner-hotkey", help="Miner SS58 hotkey (required with --host/--port if no --instance-id)")
    ...
    asyncio.run(get_logs(instance_id=instance_id, host=args.host, port=args.port, miner_hotkey=args.miner_hotkey))
